...
export class PaymentReceived {
  ...
  getCustomers() {
    this.supplierService.getAll().subscribe((res: any) => {
      ...
      // ✅ Fix: run after change detection
      setTimeout(() => {
        const routeId = this.route.snapshot.paramMap.get('customer');

        if (routeId) {
          this.paymentForm.patchValue({
            customer_id:Number(atob(routeId))
          });
  ...
  savePayment() {
    ...
    this.paymentService.create(body).subscribe(() => {
      ...
      // ✅ Optional: reset UI safely
      this.invoices.clear();
      this.summary = { received: 0, used: 0, refund: 0, excess: 0 };
