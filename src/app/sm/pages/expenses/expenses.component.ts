...
this.loading.set(true);
  ...
  ? new Date(row.expense_date.split('T')[0]) 
  : null
  ...
  reset(){
    this.form.reset();
    this.form.patchValue({ expense_date: new Date() });
    this.editMode = false;
  }
resetFilter(){
  this.fromDate = null;
...
exportExcel(){
  ...
  // basic CSV export (works instantly)
...
getCardIcon(i: number) {
  return [
    'pi-home text-blue-500',
    'pi-shopping-cart text-orange-500',
    'pi-briefcase text-cyan-500',
    'pi-wallet text-purple-500'
  ][i % 4];
}
}
